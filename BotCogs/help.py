    ...
    async def help_command(self, ctx, category: str = None):
        ...
        if cat == "locks":
            ...
            embed.add_field(
                name="Games & Schedule",
                value=(
                    "`!post_games [week]`: Post matchups (defaults to current week)\n"
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    "`!show_results <week>`: Show results without posting to a channel\n"
                    "`!show_picks <user> <week>`: Show a user's recorded picks"
                ),
                inline=False,
            )
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    "`!show_winners <week>`: Show the winners stored for a week\n"
                    "`!fix_pick <add|remove> <user> <team> <week>`: Override a pick\n"
                    "`!check_scoring_scheme`: Show this server's scoring scheme\n"
                    "`!set_scoring_scheme <all_or_nothing|additive>`: Change it\n"
                    "   Scores are computed live, so changing this rescores every past week"
                ),
                inline=False,
            )
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    "   Also answers to `!update_reactions` and `!process_existing_reactions`\n"
                    "`!repost_games <week>`: Delete and repost a week's matchups\n"
                    "`!post_results_manual <week>`: Post results for a specific week\n"
                    ...
                    "`!botstatus`: Live diagnostic snapshot (owner)\n"
                    "`!current_season` / `!list_seasons`: Season info (owner)\n"
                    "`!archive_season <year>`: Archive a past season (owner)\n"
            ...
            embed.add_field(
                name="Automation",
                value=(
                    "Tuesday 3:00 AM ET: New NFL week begins and picks open\n"
                    "Tuesday 8:00 AM ET: Winners fetched, then results posted\n"
                    "Hourly: Current week's matchups posted if they are missing\n"
                    "10-20 min before kickoff: Final pick sync, then picks lock\n"
                    "First kickoff: Picks lock and the summary posts\n"
                    "After Week 18: Season wrap-up posts\n"
                    "Aug 1 / Aug 15: Last season archived / new schedule pulled"
                ),
                inline=False,
            )
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    "`<cog>` = Cog name (e.g., reactions, winners, results)"
                ),
                inline=False,
            )
        ...
        if cat == "survivor":
            ...
            embed.add_field(
                ...
                value=(
                    ...
                    "Down to a single surviving player → that player wins\n"
                    "If every remaining player is eliminated in the same week, all of them win."
                ),
                inline=False,
            )
